...
abstract class Module {
    ...
    @Deprecated(
            "This will be removed in future versions. Payloads can be replaced via synchronous bridge methods."
    )
    ...
    /**
     * Called after [Context] has been attached to a [android.content.ContextWrapper] via
     * [android.content.ContextWrapper.attachBaseContext].
     *
     * This may be called multiple times for different [Context]s, e.g. when the app is killed by
     * the system.
     *
     ...
     *
     * If you need to register bridge methods that require an [Activity], do it in [onContext] and
     * make it async.
     *
     * Once an [Activity] is available, execute the queued calls. Then from JS, call the method
     * asynchronously.
     */
    ...
    protected fun Class<*>.method(name: String, vararg params: Class<*>?): Method =
            getDeclaredMethod(name, *params).apply { isAccessible = true }

    protected fun ClassLoader.safeLoadClass(name: String): Class<*>? =
                    ...
                    override fun beforeHookedMethod(param: MethodHookParam) {
                        ...
                        if (b != null) {
                            val scope =
                                    HookScope(
                                            param = param,
                                            proceed = { p -> super.beforeHookedMethod(p) }
                                    )
                            scope.b()
                        } else {
                    ...
                    override fun afterHookedMethod(param: MethodHookParam) {
                        ...
                        if (a != null) {
                            val scope =
                                    HookScope(
                                            param = param,
                                            proceed = { p -> super.afterHookedMethod(p) }
                                    )
                            scope.a()
                        } else {
     ...
     */
    protected class HookScope
    internal constructor(
            val param: XC_MethodHook.MethodHookParam,
            private val proceed: (XC_MethodHook.MethodHookParam) -> Unit
    ) {
        /**
         * Continues with the default XC_MethodHook super behavior. Equivalent to calling
         * `super.beforeHookedMethod(param)` or `super.afterHookedMethod(param)` depending on the
         * phase.
         */
