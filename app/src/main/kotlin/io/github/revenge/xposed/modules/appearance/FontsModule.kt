...
class FontsModule : Module() {
    ...
    override fun onLoad(packageParam: XC_LoadPackage.LoadPackageParam) =
            with(packageParam) {
                ...
                val fontDefFile =
                        File(appInfo.dataDir, "${Constants.FILES_DIR}/fonts.json").apply {
                            asFile()
                        }
                ...
                fontsDownloadsDir =
                        File(appInfo.dataDir, "${Constants.FILES_DIR}/downloads/fonts").apply {
                            asDir()
                        }
                                ...
                                async {
                                    ...
                                    try {
                                        ...
                                        val file =
                                                File(
                                                                fontsDir,
                                                                "$name${FILE_EXTENSIONS.first { url.endsWith(it) }}"
                                                        )
                                                        .apply { asFile() }
            ...
            for (fontFamilyName in fontFamilyNames) {
                try {
                    for (fileExtension in FILE_EXTENSIONS) {
                        ...
                        val file =
                                File(fontsDownloadsDir, "$customName/$refName.$fileExtension")
                                        .apply { asFile() }
                ...
                for (fontRootPath in arrayOf(fontsAbsPath, FONTS_ASSET_PATH).filterNotNull()) {
                    for (fileExtension in FILE_EXTENSIONS) {
                        ...
                        try {
                            val builder =
                                    if (fileName[0] == '/') Font.Builder(File(fileName))
    ...
    private fun createAssetTypeface(
        ...
        val fontFamilyNames =
                fontFamilyName.split(",".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
        ...
        //   For older versions of Android, ignore all the fallbacks and just use the first font
        // family
        if (fontFamilyNames.size > 1) {
            fontFamilyName =
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                        return createAssetTypefaceWithFallbacks(
                                fontFamilyNames,
                                style,
                                assetManager
                        )
        ...
        try {
            for (fileExtension in FILE_EXTENSIONS) {
                val (customName, refName) = fontFamilyName.split(":")
                val file =
                        File(fontsDownloadsDir, "$customName/$refName.$fileExtension").apply {
                            asFile()
                        }
