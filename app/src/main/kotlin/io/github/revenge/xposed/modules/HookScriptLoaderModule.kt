 ...
 *
 * Preload scripts should be placed in the [PRELOADS_DIR] directory inside the module's files
 * directory.
 *
 * The main bundle should be placed in the [Constants.CACHE_DIR] directory named
 * [Constants.MAIN_SCRIPT_FILE]. If the bundle file does not exist, it will attempt to load
 * `assets://revenge.bundle` from the module's assets.
 */
class HookScriptLoaderModule : Module() {
    ...
    private fun HookScope.runCustomScripts(
        ...
        try {
            ...
            if (mainScript.exists()) runScriptFile(mainScript)
                ...
                if (!::resources.isInitialized)
                        resources = XModuleResources.createInstance(modulePath, null)
